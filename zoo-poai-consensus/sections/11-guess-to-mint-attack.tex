...
The remainder constructs and proves the missing predicate as a \emph{software-only}
artifact---no enclave, no special hardware---that is emitted alongside genuine computation
and checked against a network-shared challenge. Reference implementations exercise its
verifier, graph emission, challenger, and a fail-closed EVM gate
(Section~\ref{sec:impl-status}); in that fixture, \texttt{AIMiner} reverts with
\texttt{ProofGateUnavailable} when no verifier is wired. This closes the one-bit-guess bug
in the reference path, not yet in live consensus. Production still requires normal
model-path emission, availability and graph coverage, and A-Chain admission, finality,
metering, nullifier, and issuance wiring. Those are load-bearing integration obligations,
not a cosmetic residual.
